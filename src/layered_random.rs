...
///
///
pub struct LayeredRandomGraph {
    n: usize,
    seed: Option<u128>,
}

impl LayeredRandomGraph {
    ...
    pub fn with_degree(self, deg: usize) -> LayeredRandomGraphRandomizer {
...
///                 .build();
/// ```
pub struct LayeredRandomGraphRandomizer {
...
impl LayeredRandomGraphRandomizer {
...
fn test_layered_graph_randomizer_determine_relative_layer_odd() {
    let lgr = LayeredRandomGraph::new(5).with_degree(3);
...
fn test_layered_graph_randomizer_determine_relative_layer_even() {
    let lgr = LayeredRandomGraph::new(6).with_degree(2);
    let actual: Vec<_> = (0..lgr.n)
        .map(|n| lgr.determine_relative_layer(n))
        .collect();
    let expected = vec![
...
fn test_layered_graph_randomizer_add_random_edge_even() {
    ...
    let mut lgr = LayeredRandomGraph::new(6).with_degree(2);
    println!("{:?}", lgr.edges);
...
fn determine_node_range_2edges_7layers_3() {
    let lgr = LayeredRandomGraph::new(7).with_degree(2);
    // third layer
...
fn determine_node_range_2edges_8layers_4() {
    let lgr = LayeredRandomGraph::new(8).with_degree(2);
    // third layer
...
fn determine_node_range_3edges_6layers_4() {
    let lgr = LayeredRandomGraph::new(6).with_degree(3);
    // third layer
    let actual = lgr.determine_vertex_indices((2, true));
